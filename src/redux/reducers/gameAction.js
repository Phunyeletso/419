...
export const handleForwardThunk =
  (playerNo, id, pos) => async (dispatch, getState) => {
    ...
    // Check if piecesAtPosition is empty or index is invalid before accessing
    const pieceIndex = piecesAtPosition.findIndex(item => item.id && item.id.slice(0, 1) == alpha);
    const piece = pieceIndex !== -1 ? piecesAtPosition[pieceIndex] : null;
    
    // If piece is null, we can't proceed with the move
    if (!piece) {
      console.error("Cannot find valid piece at position", pos);
    ...
    // Add null check for finalPlot items
    const ids = finalPlot?.map(item => item.id && item.id[0]).filter(Boolean);
    ...
    if (
      ...
      const enemyPiece = finalPlot.find(piece => piece.id && piece.id[0] !== id[0]);
      
      // Check if enemyPiece exists and has an id
      if (!enemyPiece || !enemyPiece.id) {
        console.error("Cannot find valid enemy piece");
        dispatch(unfreezeDice());
        return;
      }
